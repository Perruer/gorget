...
def is_onnx_supported() -> bool:
    ...
    if not is_supported:
        LOGGER.warning(
            ...
            "`pip install gorget[onnxruntime]` for CPU or "
            "`pip install gorget[onnxruntime-gpu]` for GPU to enable ONNX Runtime optimizations."
        )
...
def pipeline(
    ...
    **kwargs,
    ...
    if task not in get_args(ClassificationTask):
        raise GorgetValidationError(f"Invalid task. Must be one of {ClassificationTask}")
