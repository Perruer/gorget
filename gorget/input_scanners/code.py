...
class Code(Scanner):
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            GorgetValidationError: If the languages are not a subset of SUPPORTED_LANGUAGES.
        """
        if not set(languages).issubset(set(SUPPORTED_LANGUAGES)):
            raise GorgetValidationError(f"Languages must be a subset of {SUPPORTED_LANGUAGES}")
